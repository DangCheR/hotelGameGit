...
namespace XLuaTest
{
    ...
    // 如果在Lua中需要访问这个结构体，必须加上LuaCallCSharp
    [LuaCallCSharp]
    public enum MyEnum
    {
        E1,
    ...
    public class NoGc : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // 等价于 f1 = luaenv.Global.Get<IntParam>("id");
            // 生成一个原路返回的委托
            // 但这种方式不会产生gc，因为它不会产生一个LuaFunction对象，而是直接把Lua函数转换成一个delegate
            luaenv.Global.Get("id", out f1);
            Debug.Log("f1(123)=" + f1(123));
            luaenv.Global.Get("id", out f2);
            Debug.Log("f2(new Vector3(1, 2, 3))=" + f2(new Vector3(1, 2, 3)));
            luaenv.Global.Get("id", out f3);
            luaenv.Global.Get("id", out f4);
            luaenv.Global.Get("id", out f5);

            // 生成一个对调[0]和[1]的Lua函数，传入一个数组，Lua函数会对调数组的前两个元素
            luaenv.Global.Get("array_exchange", out farr);
            farr(a1);
            Debug.Log("a1[0]=" + a1[0] + ", a1[1]=" + a1[1]);
            farr(a2);
            Debug.Log("a2[0]=" + a2[0] + ", a2[1]=" + a2[1]);
            farr(a3);
            Debug.Log("a3[0]=" + a3[0] + ", a3[1]=" + a3[1]);

            luaenv.Global.Get("lua_access_csharp", out flua);

            // 生成一个接口类型的对象，这个对象需要实现IExchanger接口，Lua中定义了exchange方法，这个方法会被调用
...
//使用委托操作lua函数，传入值类型参数，lua函数直接返回这个值类型，不会产生gc
//lua中操作c#的值类型参数，直接传入这个值类型，不会产生gc
